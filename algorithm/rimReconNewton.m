...
% function [recon,memCrit] = rimReconNewton(showImages, variance , otfs , numItMax ,  L2param, Bmod , tolCGS, initGuess  )
    ...
    %---------------------------images property----------------------------------
    % showImages:
    % variance: the variance of filted and mask noise free of raw images stack
    ...
    for it = 1:numItMax
        ...
        curRes = sum( abs( cat(3,Mrec{:}) ).^2 ,3) - variance; % sum the square of otfs by z aixs, diminished by the raw image's variance
